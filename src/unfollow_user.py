...
def unfollower_users(my_following, my_followers):
    ...
    for user in my_following:
        ...
        if inactive:
            print(f"Unfollowed @{user} - Inactive for {inactive_time}+ days")
            ...
            if not_following_back and days_since_follow >= unfollow_time:
                if unfollow_user(user):
                    count += 1
                    print(f"Unfollowed @{user} - Hasn't followed back after {unfollow_time}+ days ")
                else:
                    print(f"[FAILED] Unfollowed @{user} - Hasn't followed back after {unfollow_time}+ days ")
                
            else:
                print(f"Skipped @{user} - Has {unfollow_time - days_since_follow} left to follow")

        elif not_following_back:
            if unfollow_user(user):
                count += 1
                print(f"Unfollowed @{user} - Not in temp list & Never followed")
            else:
                print(f"[FAILED] Unfollowed @{user} - Not in temp list & Never followed")
